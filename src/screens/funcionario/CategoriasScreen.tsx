...
export function CategoriasScreen() {
  const { categorias, loading, carregar, salvar, excluir, error, setError } = useCategorias();
  ...
  const handleSalvar = async () => {
    if (!nome.trim()) {
      setModalInfo('Informe o nome da categoria antes de salvar.');
      return;
    }
    ...
    if (ok) {
      ...
      setModalInfo(error ?? 'Não foi possível salvar a categoria.');
      setError(null);
    }
  };
  ...
  const handleExcluir = async () => {
    ...
    const nomeCategoria = modalExcluir.nome;
    const ok = await excluir(modalExcluir.id!);
    setModalExcluir(null);
    if (!ok) {
      setModalInfo(`A categoria "${nomeCategoria}" possui produtos vinculados. Remova ou mova os produtos para outra categoria antes de excluir.`);
      setError(null);
    }
  };
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.scroll}>
            ...
            {ICONES.map((ic) => (
              ...
              >
                <Ionicons
                  name={ic as any}
                  size={22}
                  color={iconeSelecionado === ic ? Colors.background : Colors.muted}
                />
              </TouchableOpacity>
            ))}
          </View>
      ...
      <ConfirmModal
        ...
        mensagem={`Deseja excluir "${modalExcluir?.nome}"? Esta ação não pode ser desfeita.`}
